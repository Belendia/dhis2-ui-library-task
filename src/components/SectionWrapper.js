...
import { PWDVisualizationsList } from './PWDVisualizationsList.js';
...
function Controls({ id }) {
  ...
  return (
    <>
      ...
      <p>{`Is cached: ${isCached}. Last updated: ${lastUpdated || 'n/a'}`}</p>
      <ButtonStrip>
...
export const SectionWrapper = ({ id }) => {
  return (
    <div>
      <Controls id={id} />
      <CacheableSection id={id} loadingMask={<LoadingMask />}>
        <PWDVisualizationsList />
      </CacheableSection>
    </div>
  );
};
